...
// ! 3번 문제
const words = ['apple', 'monkey', 'banana', 'pig', 'elephant'];

const filteredWords = (words, substring) => {
  // includes 메서드: 배열, 문자열 등에서 데이터 인자값이 포함되었는지를 검사(boolean 값 반환)

  return words.filter(word => word.includes(substring));
}

const containA = filteredWords(words, 'a');
console.log(containA); // [ 'apple', 'banana', 'elephant' ]