...
const number = [23, 41, 19, 7, 36];

// 배열명.forEach(각 요소에게 적용할 함수-콜백 함수)
number.forEach(function(value, index) {
  // 모든 요소에 동일하게 적용: 단순한 기능 적용에 불과
  console.log(`${index + 1}번쨰 요소: ${value}`);
});
...
// cf) 화살표 함수 적용
const newArrowNums = number.map(value => value * value);
...
const evenNmubers2 = basicArray.filter(function(value) {
  return value % 2 ===0;
})

console.log(`원래 배열 ${basicArray}`);
console.log(`짝수 배열 ${evenNumbers1}`);
...
// 전체 배열 요소 중 문자열 길이가 4보다 큰 요소만 반혼
// : 문자열.length
