...
let fruit = ['사과', '오렌지', '망고']; // 대괄호 안에 원하는 요소를 ,로 구분하여 나열
let numbers = [1,2,3,4,5];
let empty = [];
let veriableValuse = [1, '문자', true, undefined, null, [1,2,3]];

// 2) Array 생성자 사용방식
// : 새로운 배열 생성 시 배열 크기, 초기값 지정 가능

let arrayFruits1 = new Array(3);
let arrayFruits2 = new Array('사과', '야구', '농구');
...
const basketball = '농구';
// basketball = 'basket ball' - 상수 재할당 불가
...
// cf) JS 배열은 동적 배열: 임의로 뱅ㄹ의 크기를 수정
...
// 1) indexOf(): 찾는 요소의 첫 번째 인덱스를 반환 (없으면 -1)
...
newLength = fruit.unshift('mango', 'orange'); // 시작 부분에 하나 이상의 요소를 추가 (,로 나열)
...
// cf) splice(시작인덱스, 삭제 할 요소의 개수, ...전체 아이템)

console.log(fruit);
fruit.splice(1, 0, 'strawberry', 'coconut')
console.log(fruit);

// 2) 삭제: splice(시작 인덱스, 삭제할 요소의 개수)
let removedFruit = fruit.splice(1, 2);
