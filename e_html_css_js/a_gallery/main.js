...
thumbBar.addEventListener('click', (e) => {
  ...
  if(e.target.tagName === 'IMG') {
    // 요소.tagName: 해당 요소의 태그명이 전체 대문자로 변환
...
button.addEventListener('click', () => {
  ...
  // .classList: 해당 요소의 class 속성들을 배열로 반환
  // 배열 / 문자열.contains(값): 해당 값이 포함되었는지 여부 반환
  if (button.classList.contains('dark')) {
    // 현재 다크 버튼일 경우 라이트 버튼으로 변경
